...
const  Medication = () => {
	...
	return (
		<div>
			...
			<div>
				{medications.map((med) => {
					return <div key={med.medication_name}>{med.medication_name} of {med.dosage} {med.freq} times
                    <button type="submit" onClick={() => handleDelete(med.medication_name)}>remove</button>
                    </div>
				})}
			</div>
		</div>
